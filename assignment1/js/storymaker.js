...
let verbArray = [
    [`SAT ON`, `img/sat.PNG`],
    [`ATE`, `img/ate.PNG`],
    [`DANCED WITH`, `img/danced.PNG`],
    [`SAW`, `img/saw.PNG`],
    [`DOESN'T LIKE`, `img/doesnt-like.PNG`],
    [`KISSED`, `img/kissed.PNG`]
];
let adjArray = [
    [`A FUNNY`, `img/funny.PNG`],
    [`A SCARY`, `img/scary.PNG`],
    [`A GOOFY`, `img/goofy.PNG`],
    [`A SLIMY`, `img/slimy.PNG`],
    [`A BARKING`, `img/barking.PNG`],
    [`A FAT`, `img/fat.PNG`]
];
let noun2Array = [
    [`GOAT`, `img/goat.PNG`],
...
let settingArray = [
    [`ON THE MOON`, `img/moon.PNG`],
    [`ON THE CHAIR`, `img/chair.PNG`],
    [`IN MY SPAGHETTI`, `img/spaghetti.PNG`],
    [`IN MY SOUP`, `img/soup.PNG`],
    [`ON THE GRASS`, `img/grass.PNG`],
    [`IN MY SHOES`, `img/shoe.PNG`]
];
...
async function bingSpellCheck(query) {
    ...
    for (let i = 0; i < datamuseSoundsLikeJson.length; i++) {
        ...
        let tempObj = {
            [Object.values(datamuseSoundsLikeJson[i])[0]]: (Number(Object.values(datamuseSoundsLikeJson[i])[1]) * frequency)
        };
    ...
    if (datamuseSpelledLikeJson.length > 0) {
        for (let i = 0; i < datamuseSpelledLikeJson.length; i++) {
            ...
            if (!wordSet.has(Object.values(datamuseSoundsLikeJson[i])[0])) {
                ...
                wordSet.add({
                    [Object.values(datamuseSpelledLikeJson[i])[0]]: (Number(Object.values(datamuseSpelledLikeJson[i])[1]) * frequency)
                });
            }
        }
                                    ...
                                    bingSpellCheck(sentense).then(() => {
                                        if (returnresult) {
                                            ...
                                            if (typeof(defs) === "object") {
                                                ...
                                                switch (wordType) {
                                                    case 'n':
                                                        wordTypeSentense = `as a noun`;
                                                        break;
                                                    case 'v':
                                                        wordTypeSentense = `as a verb`;
                                                        break;
                                                    case 'adj':
                                                        wordTypeSentense = `as an adjective`;
                                                        break;
                                                    case 'adv':
                                                        wordTypeSentense = `as an adverb`;
                                                        break;
                                                    case 'u':
                                                        wordTypeSentense = ``;
                                                        break;
